#ifndef A_LONG_TYPE_ANALYSIS
#define A_LONG_TYPE_ANALYSIS
...
public:
	...
	void errOutputFile(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to output a file");
	}
	void errOutputDst(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to output to a non-file");
	}
	...
	void errReadFile(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to assign input to file");
	}
	...
	void errReadSrc(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to read from a non-file");
	}
	...
	void errIndexOpr(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Invalid index operation");
	}
	void errIndexOpd(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Invalid index operand");
	}
private:
