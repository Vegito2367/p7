#ifndef LEVIATHAN_DATA_TYPES
#define LEVIATHAN_DATA_TYPES
...
#ifndef LEVIATHAN_HASH_MAP_ALIAS
#define LEVIATHAN_HASH_MAP_ALIAS
...
//This class is the superclass for all leviathan types. You
...
public:
	...
	virtual const ImmutableType * asImmutable() const { return nullptr; }
...
//This DataType subclass is the superclass for all leviathan types.
...
public:
	static ArrayType * produce(const DataType * in, const int len){
		if (in == nullptr){
			throw new InternalError("array type with no subtype");
		}

		static std::list<ArrayType *> flyweights;
		for(ArrayType * fly : flyweights){
			if (fly->subType == in && fly->len == len){
				return fly;
			}
		}
		ArrayType * newType = new ArrayType(in, len);
	...
	*/
	...
	virtual const ArrayType * asArray() const override { return this; }

	virtual std::string getString() const override {
		std::string res = subType->getString();
	...
	virtual const DataType * getSub() const {
		return this->subType;
	}

	virtual size_t getSize() const override {
		return subType->getSize() * static_cast<size_t>(len);
	}
private:
	ArrayType(const DataType * sub, const int lenIn)
	: subType(sub), len(lenIn){ }
...
public:
	...
	virtual const FnType * asFn() const override { return subType->asFn(); }
	virtual const ErrorType * asError() const override { return subType->asError(); }
	virtual const ArrayType * asArray() const override { return subType->asArray(); }
	virtual bool isVoid() const override { return subType->isVoid(); }
	virtual bool isInt() const override { return subType->isInt(); }
	virtual bool isBool() const override { return subType->isBool(); }
	virtual bool isString() const override { return subType->isString(); }
	virtual bool isImmutable() const override { return true; }
	virtual bool isArray() const override { return subType->isArray(); }
	...
	virtual const ImmutableType * asImmutable() const override { return this; }
...
public:
	static TypeList * produce(const std::list<const DataType *> * typeNodes);
