...
std::string Procedure::getName() { return myName; }

Label *Procedure::getLeaveLabel() { return leaveLabel; }
...
std::string Procedure::toString(bool verbose) {
  ...
  for (auto loc : this->addrOpds) {
    res += loc->locString() + " (tmp loc of " +
           std::to_string(loc->getWidth()) + " bytes)\n";
  }
...
Label *Procedure::makeLabel() { return myProg->makeLabel(); }

void Procedure::addQuad(Quad *quad) { bodyQuads->push_back(quad); }
...
size_t Procedure::numTemps() const { return this->temps.size(); }
