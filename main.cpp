...
static leviathan::ProgramNode * parse(const char * inFile){
	...
	// AST after parsing
	leviathan::ProgramNode * root = nullptr;

	leviathan::Scanner scanner(&inStream);
	leviathan::Parser parser(scanner, &root);
...
static leviathan::NameAnalysis * doNameAnalysis(const char * inputPath){
	leviathan::ProgramNode * ast = parse(inputPath);
	if (ast == nullptr){ return nullptr; }

	return leviathan::NameAnalysis::build(ast);
}
...
static leviathan::TypeAnalysis * doTypeAnalysis(const char * inputPath){
	leviathan::NameAnalysis * nameAnalysis = doNameAnalysis(inputPath);
	if (nameAnalysis == nullptr){ return nullptr; }
	return TypeAnalysis::build(nameAnalysis);
}

static void write3AC(leviathan::IRProgram * prog, const char * outPath){
	if (outPath == nullptr){
...
static IRProgram * do3AC(const char * inputPath){
	leviathan::TypeAnalysis * typeAnalysis = doTypeAnalysis(inputPath);
...
static int writeX64(leviathan::IRProgram * prog, const char * outPath){
	if (outPath == nullptr){
