...
Opd *IntLitNode::flatten(Procedure *proc) { return LitOpd::buildInt(myNum); }
...
void ReadStmtNode::to3AC(Procedure *proc) {
  ...
  proc->addQuad(new ReadQuad(srcOpd, dstOpd, proc->getProg()->nodeType(myDst)));
}

void WriteStmtNode::to3AC(Procedure *proc) {
  if (mySrc == nullptr) {
    throw new InternalError("write mySrc null!");
  }
  if (myDst == nullptr) {
    throw new InternalError("write myDst null!");
  }

  Opd *srcOpd = mySrc->flatten(proc);
  Opd *dstOpd = myDst->flatten(proc);
  if (srcOpd == nullptr) {
    throw new InternalError("write srcOpd null!");
  }
  if (dstOpd == nullptr) {
    throw new InternalError("write dstOpd null!");
  }
  proc->addQuad(
      new WriteQuad(srcOpd, dstOpd, proc->getProg()->nodeType(mySrc)));
}
